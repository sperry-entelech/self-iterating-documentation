...
export class VersionControlEngine {
    ...
    async commit(request) {
        ...
        try {
            ...
            const versionHash = this.generateVersionHash(request.user_id, request.commit_message, timestamp);
            ...
            const { error: stateError } = await this.supabase
                ...
                .insert(stateRecords);
            if (stateError)
                throw new Error(`Failed to insert state: ${stateError.message}`);
            // 6. Log changes for audit trail
            const changeRecords = request.changes.map(change => ({
                version_id: newVersion.id,
                field_name: change.field_name,
            ...
            if (changeRecords.length > 0) {
                ...
                if (changesError)
                    throw new Error(`Failed to log changes: ${changesError.message}`);
            }
    ...
    async getVersionState(versionId) {
        ...
        if (error)
            throw new Error(`Failed to get version state: ${error.message}`);
        const state = {};
        (data || []).forEach(field => {
            state[field.field_name] = field;
        });
        return state;
    }
    /**
     ...
     */
    async getFieldHistory(fieldName, timeRange) {
      ...
      `)
        ...
        if (error)
            throw new Error(`Failed to get field history: ${error.message}`);
        return (data || []);
    }
    /**
     * Calculate diff between two versions
     */
    async diff(versionFromId, versionToId) {
        const { data, error } = await this.supabase
        ...
        if (error)
            throw new Error(`Failed to calculate diff: ${error.message}`);
    ...
    async rollback(request) {
        ...
        const rollbackMessage = `Rollback to ${targetVersion.version_hash.substring(0, 7)}: ${request.reason || targetVersion.commit_message}`;
        return this.commit({
            user_id: request.user_id,
     ...
     */
    async tagVersion(versionId, tags) {
     ...
     */
    generateVersionHash(userId, commitMessage, timestamp) {
     ...
     */
    async searchVersions(query, limit = 20) {
        const { data, error } = await this.supabase
            .from('context_versions')
            .select('*')
            .eq('user_id', this.userId)
